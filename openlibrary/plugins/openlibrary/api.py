...
class UserBookNotesAPI(delegate.page):
    ...
    def GET(self):
        ...
        return delegate.RawText(json.dumps(notes), content_type="application/json")
    ...
    def POST(self):
        ...
        # Ensure work_id is in the correct format e.g. OL45804W
        if not Work.is_valid_olid(work_id):
            raise web.HTTPError(
                "400 Bad Request", {"Content-Type": "application/json"}, data='{"error": "Invalid work_id format."}'
            )
        
        # Extract numeric part for Booknotes.add if needed, or adjust Booknotes.add to take OLID
        # For now, assuming Booknotes.add can handle OLID or has been adapted.
        # If edition_id is provided, it should also be validated or processed if needed.

        Booknotes.add(
            username=user.key.split('/')[2],
            work_id=work_id, # Assuming Booknotes.add can take full work_id like "OL45804W"
            notes=notes_text,
            edition_id=edition_id  # Pass it if provided
        )

        return delegate.RawText(json.dumps({'success': 'Note added successfully.'}), content_type="application/json")
